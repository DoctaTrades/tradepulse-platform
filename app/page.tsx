...
function AuthScreen({ onAuth }: { onAuth: (user: any) => void }) {
  ...
  const iStyle: React.CSSProperties = { width:"100%", padding:"12px 16px", background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:10, color:"var(--tp-text)", fontSize:14, outline:"none", boxSizing:"border-box" as const };
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg, #0c0e14 0%, #131620 50%, #0f1118 100%)", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Inter', sans-serif" }}>
      <div style={{ width:"min(92vw, 420px)", padding:36, background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:20, boxShadow:"0 24px 80px rgba(0,0,0,0.5)" }}>
        <div style={{ textAlign:"center", marginBottom:32 }}>
          ...
          <div style={{ fontSize:13, color:"var(--tp-muted)" }}>Your trading platform, everywhere</div>
        </div>
        ...
        <form onSubmit={mode === "login" ? handleLogin : mode === "signup" ? handleSignup : handleForgot}>
          <div style={{ marginBottom:12 }}><label style={{ fontSize:11, color:"var(--tp-muted)", textTransform:"uppercase", letterSpacing:0.8, display:"block", marginBottom:6 }}>Email</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} style={iStyle} placeholder="you@example.com" required/></div>
          {mode !== "forgot" && <div style={{ marginBottom:12 }}><label style={{ fontSize:11, color:"var(--tp-muted)", textTransform:"uppercase", letterSpacing:0.8, display:"block", marginBottom:6 }}>Password</label><input type="password" value={password} onChange={e=>setPassword(e.target.value)} style={iStyle} placeholder="••••••••" required/></div>}
          {mode === "signup" && <div style={{ marginBottom:12 }}><label style={{ fontSize:11, color:"var(--tp-muted)", textTransform:"uppercase", letterSpacing:0.8, display:"block", marginBottom:6 }}>Confirm Password</label><input type="password" value={confirmPassword} onChange={e=>setConfirmPassword(e.target.value)} style={iStyle} placeholder="••••••••" required/></div>}
        ...
        <div style={{ display:"flex", gap:8, marginBottom:16 }}>
          <button onClick={()=>handleOAuth("google")} style={{ flex:1, padding:"10px", borderRadius:8, border:"1px solid rgba(255,255,255,0.08)", background:"transparent", color:"var(--tp-faint)", cursor:"pointer", fontSize:12, fontWeight:500, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
            <svg width="16" height="16" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
            Google
          </button>
          <button onClick={()=>handleOAuth("discord")} style={{ flex:1, padding:"10px", borderRadius:8, border:"1px solid rgba(255,255,255,0.08)", background:"transparent", color:"var(--tp-faint)", cursor:"pointer", fontSize:12, fontWeight:500, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
        ...
        <div style={{ textAlign:"center", fontSize:12, color:"var(--tp-muted)" }}>
      ...
      <div className="tp-sidebar" style={{ width: collapsed ? 56 : 210, background:"var(--shell-bg2)", borderRight:"1px solid var(--border)", display:"flex", flexDirection:"column", flexShrink:0, overflow:"hidden", transition:"width 0.2s", position:"sticky", top:0, height:"100vh" }}>
            ...
            <div key={section.label}>
              ...
              {section.items.map(item => (
                <button key={item.id} onClick={()=>{if(!item.soon){setTab(item.id);setMobileOpen(false);}}} title={collapsed?item.name:undefined} style={{
                  ...
                  color: item.soon?"var(--tp-input)":tab===item.id?"var(--tp-accent-light)":"var(--tp-faint)",
        ...
        <div style={{ borderTop:"1px solid var(--border)", padding:"8px 4px" }}>
          <button onClick={()=>{setTab("settings");setMobileOpen(false);}} style={{
            ...
            color: tab==="settings"?"var(--tp-accent-light)":"var(--tp-faint)"
            ...
            {SIDEBAR.map(section => (
              <div key={section.label}>
                ...
                {section.items.filter(i=>!i.soon).map(item => (
                  <button key={item.id} onClick={()=>{setTab(item.id);setMobileOpen(false);}} style={{ display:"flex", alignItems:"center", gap:12, width:"100%", padding:"10px 14px", borderRadius:10, border:"none", background:tab===item.id?"rgba(var(--tp-accent-rgb), 0.12)":"transparent", color:tab===item.id?"var(--tp-accent-light)":"var(--tp-muted)", cursor:"pointer", fontSize:14, fontWeight:tab===item.id?600:500, marginBottom:2 }}>
