    ...
    return (
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 20px' }}>
        <div style={{ textAlign: 'center', padding: '60px 20px' }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, color: '#e2e4ea', marginBottom: 8, fontFamily: "'Rajdhani', sans-serif", letterSpacing: 1 }}>MARKET PULSE</div>
          <div style={{ fontSize: 13, color: '#5c6070', maxWidth: 450, margin: '0 auto 24px', lineHeight: 1.7 }}>
  ...
  if (loading) {
    return (
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '80px 20px', textAlign: 'center' }}>
        <div style={{ fontSize: 24, color: '#3b82f6', fontWeight: 700, fontFamily: "'Rajdhani', sans-serif" }}>⚡ Fetching market data...</div>
        <div style={{ fontSize: 12, color: '#5c6070', marginTop: 8 }}>Pulling VIX, sectors, breadth, and technicals from Schwab</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          <div style={{ fontSize: 9, color: '#5c6070', fontFamily: 'monospace', letterSpacing: 1 }}>LAST UPDATED</div>
          <div style={{ fontSize: 12, color: '#8a8f9e', fontFamily: 'monospace' }}>{new Date(data.timestamp).toLocaleString()}</div>
        </div>
        <button onClick={loadData} disabled={loading}
          style={{ padding: '8px 20px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.07)', background: 'transparent', color: '#94a3b8', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: "'Rajdhani', sans-serif", letterSpacing: 0.5, textTransform: 'uppercase' }}>
          🔄 Refresh
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr 1fr', gap: 14, marginBottom: 14 }}>
        {/* Fear & Greed Gauge */}
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '20px 16px', textAlign: 'center' }}>
          <div style={{ fontSize: 9, color: '#5c6070', textTransform: 'uppercase', letterSpacing: 1.2, fontWeight: 700, marginBottom: 8 }}>Fear & Greed</div>
          ...
          <div style={{ marginTop: 12, borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 10 }}>
            {data.fearGreed.components.map(c => (
              <div key={c.name} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, color: '#5c6070', marginBottom: 3 }}>
                <span>{c.name}</span>
        ...
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
            <div>
              <div style={{ fontSize: 9, color: '#5c6070', textTransform: 'uppercase', letterSpacing: 1.2, fontWeight: 700 }}>VIX — Fear Index</div>
          ...
          <div style={{ fontSize: 12, color: '#8a8f9e', lineHeight: 1.6 }}>{data.vix.context}</div>
        </div>

        {/* Premium Recommendation */}
        <div style={{ background: 'rgba(30,79,216,0.06)', border: '1px solid rgba(30,79,216,0.15)', borderRadius: 14, padding: '20px' }}>
          <div style={{ fontSize: 9, color: '#3b82f6', textTransform: 'uppercase', letterSpacing: 1.2, fontWeight: 700, marginBottom: 10 }}>⚡ Premium Selling Signal</div>
          <div style={{ fontSize: 13, color: '#c8cad0', lineHeight: 1.7 }}>{data.premiumRec}</div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 14 }}>
        {data.indices.map(idx => (
          <div key={idx.symbol} style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: '14px 16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 14, fontWeight: 800, color: '#e2e4ea', fontFamily: "'Rajdhani', sans-serif", letterSpacing: 1 }}>{idx.symbol}</span>
              <span style={{ fontSize: 13, fontWeight: 700, color: pctColor(idx.change), fontFamily: 'monospace' }}>{fmtPct(idx.change)}</span>
            </div>
            <div style={{ fontSize: 20, fontWeight: 700, color: '#e2e4ea', fontFamily: 'monospace', marginTop: 2 }}>{fmtPrice(idx.price)}</div>
            <div style={{ fontSize: 9, color: '#5c6070', marginTop: 4, fontFamily: 'monospace' }}>52W: {fmtPrice(idx.low52)} — {fmtPrice(idx.high52)}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
        {/* SPY Technicals */}
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '20px' }}>
          <div style={{ fontSize: 9, color: '#5c6070', textTransform: 'uppercase', letterSpacing: 1.2, fontWeight: 700, marginBottom: 12 }}>SPY Technical Health</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
            <span style={{ fontSize: 16, fontWeight: 700 }}>{trendLabels[data.spyTechnicals.trend] || '🟡 Mixed'}</span>
            <span style={{ fontSize: 12, color: '#8a8f9e', fontFamily: 'monospace' }}>RSI: {data.spyTechnicals.rsi}</span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
            {[
              ...
              <div key={ema.label} style={{ background: 'rgba(0,0,0,0.2)', borderRadius: 8, padding: '8px 10px' }}>
                <div style={{ fontSize: 9, color: '#5c6070', marginBottom: 2 }}>{ema.label}</div>
                <div style={{ fontSize: 14, fontWeight: 700, fontFamily: 'monospace', color: '#e2e4ea' }}>{ema.value ? fmtPrice(ema.value) : '—'}</div>
        ...
        {/* Breadth */}
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '20px' }}>
          <div style={{ fontSize: 9, color: '#5c6070', textTransform: 'uppercase', letterSpacing: 1.2, fontWeight: 700, marginBottom: 12 }}>Market Breadth</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: '#8a8f9e', textTransform: 'uppercase', fontFamily: "'Rajdhani', sans-serif", letterSpacing: 1 }}>
              {data.breadth.signal}
            </span>
          </div>
          <div style={{ fontSize: 12, color: '#8a8f9e', lineHeight: 1.6, marginBottom: 12 }}>{data.breadth.context}</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: 8, padding: '8px 10px' }}>
              <div style={{ fontSize: 9, color: '#5c6070' }}>RSP (Equal Weight)</div>
            ...
            <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: 8, padding: '8px 10px' }}>
              <div style={{ fontSize: 9, color: '#5c6070' }}>SPY (Cap Weight)</div>
      ...
      <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, overflow: 'hidden' }}>
        <div style={{ padding: '14px 20px', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          <span style={{ fontSize: 9, color: '#5c6070', textTransform: 'uppercase', letterSpacing: 1.2, fontWeight: 700 }}>Sector Momentum Rankings</span>
          <span style={{ fontSize: 9, color: '#3d4150', marginLeft: 8 }}>Sorted by 1-week performance</span>
        </div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              {['#', 'Sector', 'ETF', 'Price', '1D', '1W', '1M', '3M', 'RSI', 'Signal'].map(h => (
                <th key={h} style={{ padding: '8px 12px', textAlign: h === '#' || h === 'Sector' || h === 'ETF' ? 'left' : 'right', fontSize: 9, fontWeight: 600, color: '#3d4150', textTransform: 'uppercase', letterSpacing: 0.5, borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(0,0,0,0.15)' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {data.sectors.map((s, i) => {
              ...
              return (
                <tr key={s.symbol} style={{ background: i % 2 === 0 ? 'rgba(0,0,0,0.08)' : 'transparent' }}>
                  <td style={{ padding: '8px 12px', fontSize: 11, color: '#5c6070', fontFamily: 'monospace' }}>{i + 1}</td>
                  <td style={{ padding: '8px 12px', fontSize: 12, fontWeight: 700, color: '#e2e4ea' }}>{s.name}</td>
                  <td style={{ padding: '8px 12px', fontSize: 11, fontFamily: 'monospace', color: '#8a8f9e' }}>{s.symbol}</td>
                  <td style={{ padding: '8px 12px', fontSize: 11, fontFamily: 'monospace', color: '#e2e4ea', textAlign: 'right' }}>{fmtPrice(s.price)}</td>
                  ...
                  <td style={{ padding: '8px 12px', fontSize: 11, fontFamily: 'monospace', color: s.rsi > 70 ? '#f87171' : s.rsi < 30 ? '#4ade80' : '#8a8f9e', textAlign: 'right' }}>{s.rsi}</td>
