...
function buildStrategy(
  ...
  switch (strategy) {
    ...
    case 'long_straddle': {
      // Long ATM call + long ATM put at same strike
      const atmCall = pickByDelta(contracts, exp, 'CALL', 0.50);
      if (!atmCall) return [];
    ...
    case 'short_straddle': {
      // Sell ATM call + sell ATM put at same strike
      const atmCall = pickByDelta(contracts, exp, 'CALL', 0.50);
      if (!atmCall) return [];
      const strike = atmCall.strikePrice ?? 0;
      const atmPut = findContract(contracts, exp, strike, 'PUT');
    ...
    case 'long_strangle': {
      // Long OTM call (~0.30 delta) + long OTM put (~0.30 delta)
    ...
    case 'short_strangle': {
      // Sell OTM call (~0.30 delta) + sell OTM put (~0.30 delta)
      const otmCall = pickByDelta(contracts, exp, 'CALL', TARGET_SHORT_DELTA);
      const otmPut  = pickByDelta(contracts, exp, 'PUT',  TARGET_SHORT_DELTA);
      if (!otmCall || !otmPut) return [];
    ...
    case 'butterfly': {
      // Long call at lower wing + sell 2x call at center + long call at upper wing
      // Center is ATM (~0.50 delta), wings at ±width
      const atmCall = pickByDelta(contracts, exp, 'CALL', 0.50);
      if (!atmCall) return [];
      const centerStrike = atmCall.strikePrice ?? 0;
      const callStrikes = strikesForExp(contracts, exp, 'CALL');
    ...
    case 'condor': {
      // Long outer wings + sell inner body, all calls (or puts)
      // Buy lowest + sell lower-mid + sell upper-mid + buy highest
      const atmCall = pickByDelta(contracts, exp, 'CALL', 0.50);
      if (!atmCall) return [];
      const center = atmCall.strikePrice ?? 0;
      const callStrikes = strikesForExp(contracts, exp, 'CALL');
      // Inner body: ±half-width from center
      ...
      // Outer wings: width beyond inner body
...
export default function PlayBuilderModule({ user }: { user?: any }) {
  ...
  const reorderForJournal = useCallback((strategyId: StrategyId, builderLegs: Leg[]): Leg[] => {
    ...
    if (strategyId === 'condor' && builderLegs.length === 4) {
      // Order: buy lowest, sell lower-mid, sell upper-mid, buy highest
